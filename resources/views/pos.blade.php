    ...
    function displayProducts(data) {
        ...
        data.forEach(function (product, index) {
            ...
            var row = '<tr>' +
                ...
                '<td>' + product.name + '</td>' +
                '<td>' +
                '<input id="' + product.id + '-quantity" type="text" value="' + product.quantity + '" readonly style="width: 60px;"><br>' +
                '<button data-quantity="' + product.quantity + '" data-id="' + product.id + '" ' + disableIncreaseButton + ' class="badge badge-sm badge-success increase-quantity">+</button>' +
                // Add other buttons here (delete, decrease, etc.)
                '</td>' +
                '<td>' + product.price + '</td>' +
                '<td><span class="badge badge-success">' + product.sub_total + '</span></td>' +
                '<td><a href="#" class="btn btn-sm btn-primary">Detail</a></td>' +
                '<td><button data-id="' + product.id + '" class="badge badge-sm badge-danger delete-item">X</button></td>' +
                '</tr>';
